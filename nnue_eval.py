...
interpreter = Interpreter(model_path='nnue_data/connect4_model_optimized.tflite', num_threads=1) #use 1-thread for low latency  
input_details = interpreter.get_input_details()
output_details = interpreter.get_output_details()
interpreter.allocate_tensors()
output = interpreter.get_tensor(output_details[0]["index"])
interpreter.invoke()
score_ = float(output.flatten()[0])
#print("Score:", score_)
...
def features_full(board, turn):
    #print(board)
    flat = board_flat_features(board)  # 64 Felder
    #turn_feat = [turn]
    turn_feat = [1 if turn == 'w' else -1]
    mapping = {'w': 1, 'b': -1, '.': 0}
    board_np = np.vectorize(mapping.get)(board).reshape(8,8)
    #patterns = pattern_features(board) # z.B. 6 Sliding-Window-Zähler für Muster
    patterns = pattern_features(board_np)
    #print(patterns)
    #print("Flat:", flat)
    #print("Turn_feat:", turn_feat)
    #print("Patterns:", patterns)
    return np.array(flat + turn_feat + patterns, dtype=np.float32)

def board_flat_features(board):
    # Erwartet board als 2D-Array oder Liste von Listen z.B. [['w', 'b', '.', ...], ...]
    board = np.array(board)
    #print(board)
    #print("Board vor Mapping:", board)
    mapping = {'w': 1, 'b': -1, '.': 0}
    flat = np.vectorize(mapping.get)(board).flatten()
    return flat.tolist()

def pattern_features(board):
    board = np.array(board)
    #print(board)
    features = []
    for player in [1, -1]:   # 1: 'w', -1: 'b'
        features.append(count_windows(board, player, 2))
        features.append(count_windows(board, player, 3))
        features.append(count_windows(board, player, 4))
    return features

def count_windows(board, player, window_size):
    # board: 2D numpy-array mit 1 (w), -1 (b), 0 (leer)
    count = 0
    for r in range(8):
        for c in range(8 - window_size + 1):
            # Horizontal
            window = board[r, c:c+window_size]
            if all(x == player for x in window):
                count += 1
    for r in range(8 - window_size + 1):
        for c in range(8):
            # Vertikal
            window = board[r:r+window_size, c]
            if all(x == player for x in window):
                count += 1
    for r in range(8 - window_size + 1):
        for c in range(8 - window_size + 1):
            ...
            if all(x == player for x in window):
                count += 1
        for c in range(window_size - 1, 8):
